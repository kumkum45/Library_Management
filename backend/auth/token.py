...
def register_auth_routes(app):
    ...
    def signup():
        ...
        session.close()
        response = make_response(jsonify({"message": "Signup successful. Please login to get a token"}), 201)
        return add_cors_headers(response)
    ...
    def login():
        ...
        if not secret:
            session.close()
            response = make_response(jsonify({"error": "Server configuration error: SECRET_KEY not found"}), 500)
            return add_cors_headers(response)
